...
public class ChatColorMenu extends Menu {
    ...
    public List<Button> getButtons() {
        ...
        final List<Button> buttons = Arrays.stream(ChatColor.values())
                ...
                .map(color -> new ButtonBuilder(Material.WOOL)
                        .setIndex(integer.getAndIncrement())
                        .setDisplayName(color + color.name()).setData(WoolColor.getWoolColor(color))
                        .setLore(ChatColor.YELLOW + "Click to select the " + color + color.name() + ChatColor.YELLOW + " color")
                        .setAction((type, entity) -> cosmeticData.setChatColor(color))
                ).collect(Collectors.toList());

        buttons.add(new ButtonBuilder(Material.WOOL)
            .setIndex(26)
                .setData(WoolColor.getWoolColor(ChatColor.RED))
                .setDisplayName(ChatColor.RED + "Reset your chat color")
                .setAction((type, entity) -> cosmeticData.setChatColor(null))
        );
